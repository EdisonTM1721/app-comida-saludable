          ...
          builder: (context, controller, _) {
            return Text(
              controller.selectedOrder != null
                  ? 'Pedido #${(controller.selectedOrder!.orderNumber ?? 0).toString().padLeft(4, '0')}'
                  : 'Cargando pedido...',
            );
          },
        ),
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        _buildDetailRow(
                          'Pedido:',
                          '#${(order.orderNumber ?? 0).toString().padLeft(4, '0')}',
                        ),
                        _buildDetailRow('ID interno:', order.id ?? 'N/A'),
                        _buildDetailRow(
                          'Estado:',
                        ...
                        if (order.deliveredAt != null)
                          _buildDetailRow(
                            'Fecha Entrega:',
                            dateFormat.format(order.deliveredAt!.toDate()),
                          ),
                        _buildDetailRow(
                          'Método de Pago:',
                          order.paymentMethod,
                      ...
                      children: [
                        ...
                        Row(
                          ...
                          children: [
                            _buildStatusColumn(
                              'Confirmado',
                              _getTrackingStepColor(order.status, 'pending'),
                            ),
                            Expanded(
                              child: Divider(
                                color: _getTrackingStepColor(
                                  order.status,
                                  'preparing',
                                ),
                              ),
                            ...
                            _buildStatusColumn(
                              'Preparando',
                              _getTrackingStepColor(order.status, 'preparing'),
                            ),
                            Expanded(
                              child: Divider(
                                color: _getTrackingStepColor(
                            ...
                            _buildStatusColumn(
                              'En Camino',
                              _getTrackingStepColor(order.status, 'shipped'),
                            ),
                            Expanded(
                              child: Divider(
                                color: _getTrackingStepColor(
                            ...
                            _buildStatusColumn(
                              'Entregado',
                              _getTrackingStepColor(order.status, 'delivered'),
                            ),
                          ],
                        ),
                      ],
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (order.customerInfo.phoneNumber != null &&
                            order.customerInfo.phoneNumber!.isNotEmpty)
                          _buildDetailRow(
                            'Teléfono:',
                        ...
                        if (order.latitude != null && order.longitude != null) ...[
                          ...
                          Align(
                            ...
                            child: ElevatedButton.icon(
                              onPressed: () =>
                                  _openInMaps(order.latitude!, order.longitude!),
