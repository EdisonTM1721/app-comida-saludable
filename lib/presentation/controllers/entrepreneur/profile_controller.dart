...
class ProfileController extends ChangeNotifier {
  ...
  Future<void> setUserId(String? userId) async {
    ...
    if (_userId != null) {
      ...
      _errorMessage = null;
      logger.info(
        'ProfileController reseteado debido a la desconexión del usuario.',
      );
      notifyListeners();
    }
  }
  ...
  Future<void> fetchBusinessProfile() async {
    ...
    try {
      if (_userId == null) {
        ...
        _businessProfile = null;
        logger.warning(
          'fetchBusinessProfile: No hay usuario autenticado. Perfil establecido a null.',
        );
        _isLoading = false;
        notifyListeners();
        return;
      ...
      logger.info(
        'fetchBusinessProfile: Intentando obtener perfil del repositorio para el usuario $_userId...',
      );

      final BusinessProfileModel? fetchedProfile =
      await _repository.getBusinessProfile(_userId!);

      if (fetchedProfile == null) {
        _businessProfile = null;
        _errorMessage = "No se encontró un perfil de negocio para este usuario.";
        logger.info(
          'fetchBusinessProfile: No se encontró perfil en el repositorio para $_userId. _businessProfile es null.',
        );
      } else {
        _businessProfile = fetchedProfile;
        _errorMessage = null;
        logger.info(
          'fetchBusinessProfile: Perfil encontrado y asignado para $_userId. Nombre: ${_businessProfile?.name}',
        );
      }
    } catch (e, stackTrace) {
      ...
      _businessProfile = null;
      logger.severe(
        'fetchBusinessProfile: Excepción capturada durante la obtención del perfil. _businessProfile es null.',
        e,
        stackTrace,
      );
    } finally {
      _isLoading = false;
      logger.info(
        'fetchBusinessProfile: Carga finalizada. isLoading: $_isLoading, hasError: ${_errorMessage != null}, profileName: ${_businessProfile?.name}',
      );
      notifyListeners();
    }
  }
  ...
  Future<bool> saveProfile(
      BusinessProfileModel profileToSave, {
        File? imageFile,
      }) async {
    ...
    try {
      ...
      final resolvedUserId = _userId ?? currentUser?.uid;
      ...
      profileToSave = profileToSave.copyWith(userId: resolvedUserId);
      ...
      if (imageFile != null) {
        ...
        final String imagePath =
            'profile_images/$resolvedUserId/${DateTime.now().millisecondsSinceEpoch}.jpg';
      ...
      if (finalProfileToSave.id != null && finalProfileToSave.id!.isNotEmpty) {
        logger.info(
          "saveProfile: Actualizando perfil existente con ID: ${finalProfileToSave.id}",
        );
        await _repository.updateBusinessProfile(finalProfileToSave);
      } else {
        logger.info(
          "saveProfile: Creando nuevo perfil para el usuario: $resolvedUserId",
        );
      ...
      _errorMessage =
      "Error de Firebase al guardar el perfil: ${e.message ?? e.code}";
      logger.severe(
        'saveProfile: Error de Firebase. $_errorMessage',
        e,
        stackTrace,
      );
      success = false;
      ...
      logger.severe(
        'saveProfile: Error inesperado. $_errorMessage',
        e,
        stackTrace,
      );
      success = false;
      ...
      _isLoading = false;
      logger.info(
        'saveProfile: Guardado finalizado. Success: $success, isLoading: $_isLoading, hasError: ${_errorMessage != null}',
      );
      notifyListeners();
    }
